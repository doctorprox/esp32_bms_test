...
float highv = 0;
float lowv = 0;
int8_t mode = 0; // 0 = sleep, 2 = run, 4 = fault
...
void check_faults(void * parameter) {
	...
	while (true)
	{
		if (highv > fault_timer_highv_threshold) {
			fault_timer_highv++;
		}
		else
		{
		...
		if (lowv < fault_timer_lowv_threshold) {
			fault_timer_lowv++;
		}
		else
		{
